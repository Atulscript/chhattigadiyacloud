...
class HybridMagazineReader {
  constructor(containerId, options = {}) {
    ...
    this.isSubscribed = options.isSubscribed || false;
    ...
    this.options = {
      ...
      pages: options.pages || [],
      freePageLimit: this.isSubscribed ? 999 : (options.freePageLimit !== undefined ? options.freePageLimit : 5),
  ...
  setSubscribed(subscribed = true) {
    ...
    this.options.freePageLimit = this.isSubscribed ? 999 : 5;
    this.render();
  }
  ...
  goToPage(pageNum) {
    if (!this.isSubscribed && pageNum > this.options.freePageLimit) {
      ...
      <div class="reader-shell theme-${this.options.theme} font-${this.options.fontFamily} spacing-${this.options.lineSpacing} margin-${this.options.marginWidth}" 
        ...
        <header class="kindle-header-bar">
          ...
          <div class="kindle-header-right">
            ...
            ${this.isSubscribed ? `
              <a href="../../src/assets/images/mag-issue-14-cover.svg" download="Chhattisgadhiya_Cloud_Issue_14.svg" class="kindle-icon-btn kindle-download-btn" title="${t.download}" style="background:#0F9D58; border-color:#0F9D58; color:#fff;">
                <span>📥</span> <span style="font-size:0.75rem; font-weight:700;">PDF</span>
              </a>
            ` : ''}
          </div>
        </header>
        ...
        <!-- Simple Kindle Footer Controls (Uncluttered, Sleek, Unified) -->
        <footer class="kindle-simple-footer">
          <!-- Slim Reading Progress Bar Line -->
          <div class="kindle-footer-progress" title="${progressPercent}% completed">
            <div class="kindle-progress-fill" style="width: ${progressPercent}%;"></div>
          </div>

          <!-- Main Navigation & Status Row -->
          <div class="kindle-footer-nav-row">
            <button type="button" class="kindle-nav-btn" id="btn-page-prev" ${this.currentPage <= 1 ? 'disabled' : ''} aria-label="${t.prev}">
              <span class="nav-arrow">‹</span>
              <span class="nav-text">${t.prev}</span>
            </button>

            <div class="kindle-footer-status" aria-live="polite">
              <span class="footer-status-page">
                ${isHi ? `पृष्ठ <strong>${this.currentPage}</strong> / ${this.totalPages}` : `Page <strong>${this.currentPage}</strong> of ${this.totalPages}`}
              </span>
              <span class="footer-status-sep">•</span>
              <span class="footer-status-pct">${progressPercent}%</span>
              <span class="footer-status-sep hidden-mobile">•</span>
              <span class="footer-status-time hidden-mobile">${isHi ? `~${minsRemaining} मिनट शेष` : `~${minsRemaining}m left`}</span>
              ${this.isSubscribed ? `
                <span class="footer-status-sep">•</span>
                <a href="../../src/assets/images/mag-issue-14-cover.svg" download="Chhattisgadhiya_Cloud_Issue_14.svg" class="kindle-footer-pdf-link" title="${t.download}">
                  <span>📥</span> <span>${isHi ? 'डाउनलोड PDF' : 'Download PDF'}</span>
                </a>
              ` : ''}
            </div>

            <button type="button" class="kindle-nav-btn" id="btn-page-next" ${this.currentPage >= this.totalPages ? 'disabled' : ''} aria-label="${t.next}">
              <span class="nav-text">${t.next}</span>
              <span class="nav-arrow">›</span>
            </button>
          </div>

          <!-- Compact Thumbnail Jump Strip -->
          <div class="kindle-footer-thumbs" aria-label="Page Selection Strip">
            <div class="kindle-thumb-chips">
              ${thumbnailsHtml}
            </div>
          </div>
        </footer>
  ...
  nextPage() {
    if (!this.isSubscribed && this.currentPage >= this.options.freePageLimit) {
